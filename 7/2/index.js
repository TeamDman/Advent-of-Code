const fs = require("fs");
const inputTape = fs.readFileSync("./input.txt","utf8")
	.replace(/\n/g,'')
	.split(",")
	.map(x => parseInt(x));
...
let max = {in: [], out: -1};
sets:
for (const set of permute([5,6,7,8,9])) {
	...
	for(let i=0;i<10; i++) {
		for (const x of set) {
			try {
				...
				console.log(`Providing ${args}`);
				out = intcode(inputTape, args);
				console.log(`Got ${out}`);
				if (out.length === 0) {
					if (prev[0] > max.out) {
						console.log(`${prev[0]} > ${max.out}, ${set} is new max ${JSON.stringify(max)}`);
						max.out = prev[0];
						max.in = set;
					}
					break amplifying;
